...
namespace ABBuilder
{
    ...
    public class DownloadHandlerFileRange : DownloadHandlerScript
    {
        ...
        /// CurFileSize, DownloadProgress, Speed
        /// </summary>
        public event System.Action<ulong, float, float> OnProgress;
        public event System.Action<string> onFailed;
        ...
        public ulong CurDownloadedSize
        {
            get
            {
                return CurFileSize;
        ...
        public DownloadHandlerFileRange(string path, UnityWebRequest request, string tmpVariant = null) : base(new byte[1024 * 1024])
        {
            Debug.Log($"[Download] DownloadHandlerFileRange start");
            Path = path;
            //if(!string.IsNullOrEmpty(tmpVariant))
            //    Path += "." + tmpVariant;
            if (!string.IsNullOrEmpty(tmpVariant))
                Path += tmpVariant;
        ...
        private void Clean()
        {
            ...
            //Debug.Log($"CurFileSize={CurFileSize},TotalFileSize={TotalFileSize},LastDataSize={LastDataSize},LocalFileSize={LocalFileSize}");
            //if (CurFileSize != TotalFileSize)
            //{
            //    FileInfo finfo = new FileInfo(Path);
            //    var newpath = Path.Insert(Path.Length - ".tmp".Length, $"[{CurFileSize}]");
            //    Debug.Log($"move to new path: " + newpath);
        ...
        //[Obsolete]Note:当下载的文件数据大于2G时,该int类型的参数将会数据溢出,所以先自己通过响应头来获取长度,获取不到再使用参数的方式
        protected override void ReceiveContentLengthHeader(ulong contentLength)
        {
            ...
            if (!string.IsNullOrEmpty(contentLengthStr))
            {
                ...
                catch (System.FormatException e)
                {
                    ...
                    onFailed?.Invoke(unityWebRequest.url + "\n" + e.Message);
                }
        ...
        {
            if (isPaused) { Debug.Log("Stopped"); return false; }
            if (!Application.isPlaying) { Debug.Log("app closed"); Clean(); return false; }
            if (data == null || dataLength == 0 || unityWebRequest.responseCode > 400)
            {
                onFailed?.Invoke(Path + "\n" + unityWebRequest.error);
                Debug.Log($"[Download] ReceiveData [{Path}] return false: data: {data} length: {dataLength} responseCode: {unityWebRequest.responseCode}");
                return false;
            }
            ...
            OnProgress?.Invoke((ulong)CurFileSize, DownloadProgress, Speed);
